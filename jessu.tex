...
% @Last Modified time: 2022-08-12 21:48:28
...
\begin{algorithm}\label{prohand}
	...
	\KwOut{authoritative blockchain $A_v$}
	\caption{Handling Inconsistent Blockchain Views ($G$)}
	\tcp*[h]{Collecte all valid sibling groups to  \(S\)} \\
	$S \gets \varnothing$ \\
	\ForEach { $B \in G$}{
		$s \gets \Call{Siblings}{B}$ \\
			$S \gets S \cup s$ 
	}
	...
	$s \gets \Call{Siblings}{B^0}$
	\uIf{ $\Call{ ChildSiblingGroups}{s} = \varnothing$ \label{alg:goto_childsubgroup}}
	{
		\KwRet $s$ \\
	}
	\Else
